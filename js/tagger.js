const TaggerModule = (() => {
  ...
  // Restores existing tags from state.tags so back-navigation works.
  function render(container, audioBlob) {
    ...
    // Restore tags that already exist (e.g. user pressed Back from screen7)
    (_state.tags || []).forEach(tag => {
      const list = document.getElementById('tg-tag-list');
      if (!list) return;
      const item = document.createElement('div');
      item.className  = 'tg-tag-item';
      item.dataset.id = tag.id;
      item.dataset.ts = tag.timestamp;
      ...
      <div class="tagger-wrap">
        <div class="tagger-player">
          <div class="tagger-player-info">
            <span class="tagger-duration-label">Your recording · tap bar to seek</span>
  ...
  // Seekable progress bar — works before first play (creates audio + waits for metadata)
  ...
  function addTag() {
    ...
    const ts = audio.readyState >= 1 ? audio.currentTime : 0;
    const id = Date.now();
    ...
    item.innerHTML = `
      ...
      <input class="tg-tag-input text-input" type="text" placeholder="Describe the issue…" maxlength="80">
  ...
  function seekToTag(ts) {
    const audio = getOrCreateAudio();
    const doSeek = () => { audio.currentTime = ts; };
    if (audio.readyState >= 1) doSeek();
    else audio.addEventListener('loadedmetadata', doSeek, { once: true });
  }

  function _renderTagItem(el, tag) {
    el.className = 'tg-tag-item';
    el.innerHTML = `
  ...
  function _placeMarker(tag) {
    const duration = (_audio && _audio.duration) || (_state.recordingTime || 1);
  ...
  function updateSaveBadge() {
    ...
    if (!btn) return;
    const base = (_state.savedSessionId) ? 'Update session' : 'Save session';
    const n    = _state.tags.length;
    btn.textContent = n ? `${base} (${n} tag${n > 1 ? 's' : ''})` : base;
  }
  ...
  async function saveSession() {
    ...
    try {
      if (_state.savedSessionId) {
        // Already saved — just update the tags
        await DB.updateSessionTags(_state.savedSessionId, sessionData.tags);
      } else {
        const id = await DB.saveSession(sessionData, _audioBlob);
        _state.savedSessionId = id;
      }
      _state.tags = sessionData.tags;
      if (btn) { btn.disabled = false; btn.textContent = 'Saved ✓'; }
      refreshExcerptBadge();
      setTimeout(() => goTo('screen7'), 700);
    } catch (err) {
  ...
  function discardSession() {
    const msg = _state.tags.length
      ? `Discard recording and ${_state.tags.length} tag(s)?`
      : 'Discard this recording?';
    if (confirm(msg)) _doDiscard();
  }

  function _doDiscard() {
    _state.tags          = [];
    _state.savedSessionId = null;
    stop();
    goTo('screen7');
  }
